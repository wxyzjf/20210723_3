/opt/etl/prd/etl/APP/ADW/Y_B_RETENT_CORP_COMM_RPT/bin> cat b_retent_corp_comm_rpt0030.pl
######################################################
...
execute ${etlvar::UTLDB}.ETL_UTILITY.TRUNCATE_TBL(p_table_schema=>'${etlvar::MIGDB}',p_table_name=>'B_RETENT_CORP_COMM_003A_T');
execute ${etlvar::UTLDB}.ETL_UTILITY.TRUNCATE_TBL(p_table_schema=>'${etlvar::MIGDB}',p_table_name=>'B_RETENT_CORP_COMM_003A02_T');
execute ${etlvar::UTLDB}.ETL_UTILITY.TRUNCATE_TBL(p_table_schema=>'${etlvar::MIGDB}',p_table_name=>'B_RETENT_CORP_COMM_003A03_T');
execute ${etlvar::UTLDB}.ETL_UTILITY.TRUNCATE_TBL(p_table_schema=>'${etlvar::MIGDB}',p_table_name=>'B_RETENT_CORP_COMM_003B01_T');
execute ${etlvar::UTLDB}.ETL_UTILITY.TRUNCATE_TBL(p_table_schema=>'${etlvar::MIGDB}',p_table_name=>'B_RETENT_CORP_COMM_003C_T');
execute ${etlvar::UTLDB}.ETL_UTILITY.TRUNCATE_TBL(p_table_schema=>'${etlvar::MIGDB}',p_table_name=>'B_RETENT_CORP_COMM_003_T');


prompt 'b_retent_corp_comm_003A_t [Preparing old ld billing first serval bill inv list within the valid ld period]';

insert into mig_adw.b_retent_corp_comm_003a_t(
         rpt_mth
        ,comm_mth
        ,case_id        
        ,old_ld_inv_num
...
insert into  mig_adw.b_retent_corp_comm_003a02_t
(
         rpt_mth
        ,comm_mth
        ,case_id 
...
from mig_adw.b_retent_corp_comm_003a_t t
...
insert into  mig_adw.b_retent_corp_comm_003a03_t
(
         rpt_mth
        ,comm_mth
        ,case_id
...
from(
    Select 
        ...
        ,'SAL_LED_HIST' 
    from mig_adw.b_retent_corp_comm_003a02_t t
...
insert into mig_adw.B_RETENT_CORP_COMM_003B01_T
(       rpt_mth
        ,comm_mth
        ,case_id
        ,old_ld_inv_num
...
insert into mig_adw.b_retent_corp_comm_003c_t
(
        case_id
       ,mkt_cb_amt
        ,json_rmk
...
insert into mig_adw.B_RETENT_CORP_COMM_003_T
(
    rpt_mth
    ,comm_mth
    ,case_id
...
from mig_adw.B_RETENT_CORP_COMM_001_T t
left outer join mig_adw.B_RETENT_CORP_COMM_003A03_T inv
        on t.case_id = inv.case_id
left outer join mig_adw.B_RETENT_CORP_COMM_003B01_T hs
        on t.case_id = hs.case_id
left outer join mig_adw.B_RETENT_CORP_COMM_003C_T oc
