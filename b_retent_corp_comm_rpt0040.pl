/opt/etl/prd/etl/APP/ADW/Y_B_RETENT_CORP_COMM_RPT/bin> cat b_retent_corp_comm_rpt0040.pl
######################################################
...
execute ${etlvar::UTLDB}.ETL_UTILITY.TRUNCATE_TBL(p_table_schema=>'${etlvar::MIGDB}',p_table_name=>'B_RETENT_CORP_COMM_004A01_T');
execute ${etlvar::UTLDB}.ETL_UTILITY.TRUNCATE_TBL(p_table_schema=>'${etlvar::MIGDB}',p_table_name=>'B_RETENT_CORP_COMM_004B01_T');
...
insert into  mig_adw.B_RETENT_CORP_COMM_004A01_T t
(
   rpt_mth
  ,comm_mth
  ,case_id
...
left outer join mig_adw.B_RETENT_CORP_COMM_002_T n 
        on t.case_id = n.case_id
---old rebate hs_subsidy
left outer join mig_adw.B_RETENT_CORP_COMM_003_T o
        on t.case_id = o.case_id
left outer join mig_adw.RETENT_CORP_COMM_MKT_REF bmc
        on t.ld_mkt_cd = bmc.mkt_cd
           and bmc.COMM_MTH = &comm_mth
left outer join mig_adw.RETENT_CORP_COMM_MKT_REF bmc2
        on t.pos_mkt_cd = bmc2.mkt_cd
           and bmc2.COMM_MTH = &comm_mth
left outer join (
---Bm cat file base on invoice date
        Select tr.case_id,b.bm_cat
        from mig_adw.B_RETENT_CORP_COMM_001_T tr
            ,prd_adw.pos_inv_header ph
            ,mig_adw.RETENT_CORP_COMM_MKT_REF b
...
update  mig_adw.B_RETENT_CORP_COMM_004A01_T t
set (t.old_rebate_amt,t.old_bill_rate,t.rebate_amt,t.bill_rate,rebate_flg,old_rebate_flg,old_ld_inv_num) = (
        select  r.overr_rebate_amt 
                ...
                ,'INV_REBATE'
                ,'OVERRIDE_OLD'
        from mig_adw.RETENT_CORP_COMM_MKT_REF r
        where t.ld_mkt_cd = r.mkt_cd 
         and nvl(r.override_flg,' ') ='Y'
         and r.comm_mth = &comm_mth
)where t.ld_mkt_cd 
in ( select rr.mkt_cd from mig_adw.RETENT_CORP_COMM_MKT_REF rr where nvl(rr.override_flg,' ') ='Y' and rr.comm_mth = &comm_mth) ;
...
declare
    cursor cur_t is
        select * from mig_adw.B_RETENT_CORP_COMM_004A01_T ;
    rs_sr  mig_adw.B_RETENT_CORP_COMM_004A01_T%ROWTYPE;
    rs_ta  mig_adw.B_RETENT_CORP_COMM_004B01_T%ROWTYPE;
    commit_cnt integer;
...
-----notice rebate_amt is a negative figure----
    ...
    insert into mig_adw.B_RETENT_CORP_COMM_004B01_T
        (
     rpt_mth
    ,comm_mth
    ,case_id
...
insert into MIG_ADW.RETENT_CORP_COMM_H
(
     rpt_mth
    ,comm_mth
    ,case_id
...
select 
  ...
  ,pos_ld_cd
    ...
    ,salesman_cd
    ,json_rmk
from  mig_adw.B_RETENT_CORP_COMM_004B01_T;
commit;
