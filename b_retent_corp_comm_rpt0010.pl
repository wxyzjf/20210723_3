/opt/etl/prd/etl/APP/ADW/Y_B_RETENT_CORP_COMM_RPT/bin> cat b_retent_corp_comm_rpt0010.pl
######################################################
...
execute ${etlvar::UTLDB}.ETL_UTILITY.TRUNCATE_TBL(p_table_schema=>'${etlvar::MIGDB}',p_table_name=>'B_RETENT_CORP_COMM_001A01_T');
execute ${etlvar::UTLDB}.ETL_UTILITY.TRUNCATE_TBL(p_table_schema=>'${etlvar::MIGDB}',p_table_name=>'B_RETENT_CORP_COMM_001A02_T');
execute ${etlvar::UTLDB}.ETL_UTILITY.TRUNCATE_TBL(p_table_schema=>'${etlvar::MIGDB}',p_table_name=>'B_RETENT_CORP_COMM_001A03_T');
execute ${etlvar::UTLDB}.ETL_UTILITY.TRUNCATE_TBL(p_table_schema=>'${etlvar::MIGDB}',p_table_name=>'B_RETENT_CORP_COMM_001A04_T');
execute ${etlvar::UTLDB}.ETL_UTILITY.TRUNCATE_TBL(p_table_schema=>'${etlvar::MIGDB}',p_table_name=>'B_RETENT_CORP_COMM_001A05_T');
...
insert into mig_adw.b_retent_corp_comm_001A01_t
(
     RPT_MTH
    ,COMM_MTH
    ,CASE_ID
...
insert into mig_adw.b_retent_corp_comm_001A02_t
(
     RPT_MTH
    ,COMM_MTH
    ,CASE_ID
...
select     
    ...
    ,t.JSON_RMK
from mig_adw.b_retent_corp_comm_001A01_t t
...
Select   
    ...
    from mig_adw.b_retent_corp_comm_001A01_t t
        ,mig_adw.b_retent_corp_comm_001A01_t t2
...
Select
        ...
        ,t.json_rmk
    from 
        mig_adw.b_retent_corp_comm_001A01_t t
        ,prd_adw.mkt_ref_vw mk
    where
       t.pos_mkt_cd <>'CSTPN'
      ...
      and t.pos_ld_cd <> ' '
      and t.pos_inv_num not in (select pos_inv_num from mig_adw.b_retent_corp_comm_001A02_t)
      and t.pos_mkt_cd in (Select mkt_cd from mig_adw.RETENT_CORP_COMM_MKT_REF where COMM_MTH = &comm_mth);
commit;
...
select
    ...
    ,sl.ld_expired_date
    ,t.json_rmk    
from  mig_adw.b_retent_corp_comm_001A03_t t
...
where t.cust_num = sl.cust_num
  ...
  and t.case_id not in ( select r.case_id from mig_adw.B_RETENT_CORP_COMM_001A04_T r)
group by 
     t.rpt_mth
    ,t.comm_mth
    ,t.case_id
...
from  mig_adw.b_retent_corp_comm_001A03_t t
where  t.case_id not in ( select r.case_id from mig_adw.B_RETENT_CORP_COMM_001A04_T r);
commit;
...
select
        ...
        ,t.json_rmk
from    mig_adw.b_retent_corp_comm_001A04_t t
...
insert into mig_adw.B_RETENT_CORP_COMM_001A05_T
    (   case_id
...
update mig_adw.b_retent_corp_comm_001A_t t
set t.subr_sw_on_date
=(select min(r.overr_subr_sw_on_date) from mig_adw.B_RETENT_CORP_COMM_001A05_T r where r.case_id = t.case_id)
where t.case_id in (
        select rr.case_id from mig_adw.B_RETENT_CORP_COMM_001A05_T rr
);
commit;
...
--  Select t.case_id 
--  from
--    mig_adw.b_retent_corp_comm_001A_t t 
...
from (
        ...
        where t.subr_num = o.subr_num
         ...
         and t.ld_mkt_cd not in (select rr.mkt_cd from mig_adw.RETENT_CORP_COMM_MKT_REF rr where upper(rr.BM_CAT) = 'OHTERS' and rr.COMM_MTH = &comm_mth)
        group by t.rpt_mth
            ,t.comm_mth
            ,t.case_id
            ,t.ld_inv_num
